    ...
    # Legge un frame dal video
        ...
        if circles is not None:
            ...
            for(x,y,r) in circles:
                ...
                mod = math.sqrt((posizione_x-posizione_x_prec)**2 + (posizione_y-posizione_y_prec)**2)
...
def test_servo(ang_min, ang_max):
  ...
  thread = threading.Thread(target=muovi_avanti, args=(SERVO_PIN,ang_min, ang_max,))
  thread1 = threading.Thread(target=muovi_indietro, args=(SERVO_PIN1,ang_min, ang_max,))
  thread.start()
  thread1.start()
  thread.join()
  thread1.join()
  
  thread1 = threading.Thread(target=muovi_avanti, args=(SERVO_PIN1,ang_min, ang_max,))
  thread2 = threading.Thread(target=muovi_indietro, args=(SERVO_PIN2,ang_min, ang_max,))
  thread1.start()
  thread2.start()
  thread1.join()
  thread2.join()
  
  thread = threading.Thread(target=muovi_indietro, args=(SERVO_PIN,ang_min, ang_max,))
  thread2 = threading.Thread(target=muovi_avanti, args=(SERVO_PIN2,ang_min, ang_max,))
  thread.start()
  thread2.start()
  thread.join()
  thread2.join()
